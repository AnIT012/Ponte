...
- `job`: 下の層のプログラム（学習、変換など）を約束付きで走らせる。`with` の値は引数になり、プログラムは事実を `report()` で報告する。`confirm` と `suspect`（良すぎる数字など、おかしいときに止める条件）は走っている間に照合して、その場で止める。`require`（満たすべき条件）は終わったあとに照合する（仕様 5章 job）
