...
### U1. 足した語（仕様書 6章「画面のための追補」）
tabs、this as detail、empty、part の中の計算、days until、match の数の範囲、tone、`#色`。全部「要確認」として仕様書に書いた。外したいものがあれば言ってください。

### U2. タップを thing の名前で受ける
- 仮: `user taps X on Application` と書けば、どの一覧・ボード・詳しくから押しても動く。list の名前で書いた rule もそのまま動く。

### U3. words に無い文字
- 仮: `words` に書いた語だけ訳す。spec の中にそのまま書いた日本語（`"あと{left}日"`、ボタンの `named 提出した`）は英語でも日本語のまま。全部訳したいなら、`named add` のように words の名前で書く。

### U4. 過ぎた締切の数え方
- 仮: `days until` は日付だけで数える（時刻は見ない）。今日の23:59までの締切は 0（今日）。
