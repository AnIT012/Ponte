...
**2026-09-23: おすすめ通りで進めることになり、下の「対応」の通りにした。B2・B6 も B で対応した。残っているのは B8 と、C の connect does・map / media だけ。**
...
| B2 | 年をまたぐ締切の決まり（**対応: 今日より前なら「来年ですか？」と聞き返す。はいなら年まで入れて保存**） | 12月に来年1月の締切を足すと、今の年として読んでしまう。tbd にさせるか、「今より前なら来年」と決めるか | 小 |
...
| B6 | 元に戻す（**対応: 自分がした直前の1回の move だけ、「取り消す」で戻せる。他の箱も動いた時は出さない**） | flow は戻れないので、押し間違いを直せない。flow に戻る矢印を書くか、「直前の move だけ取り消せる」を言語で持つか | 中 |
