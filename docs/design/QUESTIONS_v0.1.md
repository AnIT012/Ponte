...
### Q19. `within 3 days` は72時間？ 暦日？（Go変換で見つかった）
仕様の example: `deadline "9/24 23:59"`, `at "9/21 21:00"` で `expect notify`。
72時間ぴったりなら 9/24 21:00 までなので**通知されない**。example が通るのは「暦日で3日後の終わりまで」の解釈だけ。
- 仮: days / weeks は暦日（N日後の 23:59:59 まで）、hours / minutes は時間ぴったり。過ぎたものは含めない（now 以降）。
- 質問: この解釈でいい？ 過ぎた締切（now より前）は DueSoon に入れる？
