...
「3日以内」は日付で数えます。今の時刻から、3日後の日付の終わり（23:59）までです。締切がもう過ぎたものは入れません。
...
全角の数字や記号（例「１０／１５（木）１２：００」）でも、半角と同じ答えにしてください。年は推測しないでください。自信がないとき（見つからないとき）は None を返してください（ユーザーに聞き返す合図です）。
